...
def generate_synthetic_data(num_rows=1000):
    ...
    for _ in range(num_rows):
        ...
        data.append({
            "gender": gender, "age_group": random.choice(age_groups),
            "occasion": random.choice(occasions), "skin_tone": random.choice(skin_tones),
            "style": random.choice(styles), "item": item, "brand": random.choice(brands),
            "color": color, "price": random.randint(20, 500),
            "image_url": image_url, "product_url": "https://amazon.in"
        })
