...
import { Home, Sliders, Scan, Activity, Loader2, UploadCloud, Sun, Moon, CheckCircle2, Terminal } from "lucide-react";
...
export default function NovaPlatform() {
  ...
  return (
    <main className="min-h-screen bg-zinc-50 dark:bg-[#0a0a0c] text-zinc-900 dark:text-zinc-100 font-sans selection:bg-blue-500/30">
            ...
            <div className="grid lg:grid-cols-2 gap-12 items-center min-h-[60vh]">
              <div className="space-y-8 relative z-10">
                ...
                <div className="flex gap-4">
                  <button onClick={() => setActiveTab("Vision Setup")} className="bg-blue-600 hover:bg-blue-700 text-white px-8 py-4 rounded-xl font-bold transition-all shadow-lg shadow-blue-500/25 flex items-center">
              ...
              <div className="relative h-[500px] w-full flex items-center justify-center">
                ...
                <div className="absolute top-10 left-10 bg-white/80 dark:bg-zinc-900/80 backdrop-blur-md border border-zinc-200 dark:border-zinc-800 p-3 rounded-xl shadow-xl animate-bounce">
                  <p className="text-xs font-mono font-bold text-blue-500">CV_MODE: ACTIVE</p>
                </div>
                <div className="absolute bottom-10 right-10 bg-white/80 dark:bg-zinc-900/80 backdrop-blur-md border border-zinc-200 dark:border-zinc-800 p-3 rounded-xl shadow-xl animate-bounce" style={{animationDelay: "1s"}}>
                  <p className="text-xs font-mono font-bold text-purple-500">KMEANS_K=3</p>
                </div>
              </div>
            </div>
          )}
            ...
            <div className="space-y-6 max-w-5xl mx-auto">
              <div className="bg-white dark:bg-zinc-900 p-8 rounded-[2rem] border border-zinc-200 dark:border-zinc-800 shadow-sm relative overflow-hidden">
                ...
                <div className="mt-8 flex justify-between items-end">
                  <div className="hidden md:flex flex-col space-y-1 w-1/2">
                    {logs.map((log, i) => <p key={i} className="text-[10px] font-mono text-zinc-400 dark:text-zinc-500">[{new Date().toLocaleTimeString()}] {log}</p>)}
                  </div>
                  <button onClick={handleManualSubmit} disabled={loadingManual} className="bg-blue-600 text-white font-bold py-3 px-8 rounded-xl hover:bg-blue-700 transition-all flex items-center shadow-lg shadow-blue-500/20">
              ...
              {recommendations.length > 0 && (
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6 animate-in fade-in slide-in-from-bottom-4">
                  {recommendations.map((item, idx) => (
                    <div key={idx} className="bg-white dark:bg-zinc-900 rounded-[2rem] overflow-hidden border border-zinc-200 dark:border-zinc-800 shadow-sm hover:border-blue-500/50 transition-colors group">
          ...
          {(activeTab === "Vision Setup" || activeTab === "Fit Rater") && (
            <div className="grid lg:grid-cols-5 gap-6 max-w-6xl mx-auto">
              <div className="lg:col-span-2">
                <div className={`bg-white dark:bg-zinc-900 border-2 border-dashed ${loadingVision || loadingFit ? 'border-blue-500 shadow-[0_0_30px_rgba(59,130,246,0.2)]' : 'border-zinc-300 dark:border-zinc-700'} rounded-[2rem] p-8 flex flex-col items-center justify-center min-h-[500px] relative overflow-hidden group transition-all duration-300`}>
                  {(activeTab === "Vision Setup" && visionPreview) || (activeTab === "Fit Rater" && fitPreview) ? (
                    <>
                      <img src={activeTab === "Vision Setup" ? visionPreview! : fitPreview!} className="absolute inset-0 w-full h-full object-cover opacity-60" />
                      {(loadingVision || loadingFit) && (
                        <div className="absolute inset-0 bg-blue-500/20 backdrop-blur-sm flex flex-col items-center justify-center">
                          <div className="w-full h-1 bg-blue-500/50 absolute top-0 animate-[ping_2s_linear_infinite]"></div>
                    ...
                    <div className="text-center z-10">
                      <div className="h-20 w-20 bg-blue-50 dark:bg-blue-900/20 rounded-2xl flex items-center justify-center mx-auto mb-6 text-blue-500 group-hover:scale-110 transition-transform">
                        <UploadCloud size={32} />
                      </div>
                      <h3 className="text-xl font-bold mb-2">Initialize Image Data</h3>
                      <p className="text-zinc-500 text-sm">Tap or drop media here</p>
                    </div>
                  )}
                  <input type="file" accept="image/*" className="absolute inset-0 opacity-0 cursor-pointer z-20" onChange={activeTab === "Vision Setup" ? handleVisionUpload : handleFitUpload} />
                </div>
              </div>
                  ...
                  visionData && !loadingVision ? (
                    <div className="space-y-8 animate-in fade-in slide-in-from-right-8">
                      <div className="flex items-center space-x-2 text-green-500 bg-green-500/10 px-3 py-1.5 rounded-lg w-max mb-2 border border-green-500/20">
                        <CheckCircle2 size={16} /><span className="text-xs font-bold uppercase tracking-widest">DeepFace Pipeline Complete</span>
                      </div>
                     ...
                     <div className="m-auto text-center text-zinc-400 max-w-sm"><Terminal size={48} className="mx-auto mb-4 opacity-20" /><p className="text-sm">Awaiting image payload to initiate DeepFace and OpenCV color clustering pipeline.</p></div>
                  )
                ) : (
                  fitData && !loadingFit ? (
